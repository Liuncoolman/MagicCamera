...
public class TwoLineSeekBar extends View {
	...
	protected void onDraw(Canvas canvas) {
        if(mSeekLength == 0) {
            int width = getWidth();
            mSeekLength = ((width - getPaddingLeft()) - getPaddingRight() - (mThumbRadius * 2.0f));
            ...
            if((mDefaultValue == 0) || (mDefaultValue == mMaxValue)) {
                ...
                if(currValue < mDefaultValue) {
                    ...
                    mThumbOffset = ((((mSeekLength - defaultAreaLength) * currValue) / mMaxValue) + (mDefaultAreaRadius * 2.0f));
                } else {
                    mThumbOffset = mNailOffset;
                }
            }
        }
        float top = getMeasuredHeight() / 0x2 - (mLineWidth / 2.0f);
    ...
    private void setValueInternal(int value) {
        ...
        mCurrentValue = value;
        if(mListener != null) {
            mListener.onSeekChanged(((mStartValue + value) * mStep), mStep);
        }
    }
